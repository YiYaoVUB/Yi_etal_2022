...
csvframe_noi = pd.read_csv("D:\Tower_result\Global_simulations\\NOI_final.csv",header=None)
csvframe_ctl = pd.read_csv("D:\Tower_result\Global_simulations\\CTL_final.csv",header=None)
csvframe_irr = pd.read_csv("D:\Tower_result\Global_simulations\\IRR_final.csv",header=None)
...
plt.ylabel(r"Changes ($W/m^2$)", fontsize=20)
...
plt.legend(loc='upper left', fontsize=20,frameon=False)
ax1.set_title('CTL - NOI',loc='right',fontsize = 22)
plt.ylabel(r"Changes (mm/year) / (kg/$m^2$)", fontsize=20)
...
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\Region-impacts.png')
#plt.show()
print('test')