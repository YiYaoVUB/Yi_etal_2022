...
str_middle = ["CTL", "IRR_drai", "IRR_meth", "IRR_pool", "IRR_satu"]
...
for exp = 1 : len_expe
    ...
    for index = 1 : len_countries
        ...
        if (Qirr_obs_world(index, 3))% && QIRRIG_COUNTRY(index, 2))
            sum_bias = sum_bias + (QIRRIG_COUNTRY(index, 17) - Qirr_obs_world(index, 3)) * Qirr_obs_world(index, 3)
            sum_rmse = sum_rmse + (QIRRIG_COUNTRY(index, 17) - Qirr_obs_world(index, 3))^2 * Qirr_obs_world(index, 3)
            sum_qirrig = sum_qirrig + Qirr_obs_world(index, 3)
        end
    end
    bias_aqua(exp) = sum_bias / sum_qirrig
    rmse_aqua(exp) = (sum_rmse/ sum_qirrig)^(1/2)
    ...
    for index = 1 : len_province
        ...
        sum_bias = sum_bias + (QIRRIG_PROVINCE(15, index) - Qirr_obs_china(30, index)) * Qirr_obs_china(30, index)
        sum_rmse = sum_rmse + (QIRRIG_PROVINCE(15, index) - Qirr_obs_china(30, index))^2 * Qirr_obs_china(30, index)
        sum_qirrig = sum_qirrig + Qirr_obs_china(30, index)
        
    end
    bias_china(exp) = sum_bias / sum_qirrig
    rmse_china(exp) = (sum_rmse / sum_qirrig)^(1/2)
    ...
    for index = 1 : len_state
        ...
        if (Qirr_obs_usa(index, 4))% && QIRRIG_STATE(index, 5))
            sum_bias = sum_bias + (QIRRIG_STATE(index, 15) - Qirr_obs_usa(index, 4)) * Qirr_obs_usa(index, 4)
            sum_rmse = sum_rmse + (QIRRIG_STATE(index, 15) - Qirr_obs_usa(index, 4))^2 * Qirr_obs_usa(index, 4)
            sum_qirrig = sum_qirrig + Qirr_obs_usa(index, 4)
        end 
    end
    bias_usa(exp) = sum_bias / sum_qirrig
    rmse_usa(exp) = (sum_rmse/ sum_qirrig)^(1/2)
end
