...
data_E_NOI = Data_from_nc('D:\Tower_result\Global_simulations\\surfdata_0.9x1.25_hist_78pfts_CMIP6_simyr2000_c210203.nc')   #load the data
data_variable_E = data_E_NOI.load_variable('irrigation_method')
data_variable_E_NOI = data_variable_E[1, :, :]
data_variable_E_NOI = data_variable_E_NOI[29 :, :]
data_lon = data_E_NOI.load_variable('LONGXY')
data_lon = data_lon[0,:]
data_lat = data_E_NOI.load_variable('LATIXY')
data_lat = data_lat[:,0]
data_lat = data_lat[29 :]
data_variable_E_IRR = data_variable_E[47, :, :]
data_variable_E_IRR = data_variable_E_IRR[29 :, :]


f = plt.figure(figsize = (12, 3.5), dpi=100)  # initiate the figure
set_plot_param()
...
h = ax1.pcolormesh(data_lon,data_lat,data_variable_E_NOI, cmap=cmap_bias1, rasterized=True, norm=norm_bias1)
...
ax1.set_title('wheat',loc='right',fontsize = 12)
...
h = ax1.pcolormesh(data_lon,data_lat,data_variable_E_IRR, cmap=cmap_bias1, rasterized=True, norm=norm_bias1)
cbar   = f.colorbar(h, ax=ax1, cmap=cmap,
                               spacing='uniform',
                               orientation='horizontal',
                               extend='neither', shrink = 0.8, pad = 0, aspect = 50)
...
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\IrrMeth.png')