...
#IPCC = gp.GeoDataFrame.from_file('E:\Data_Evaluation\ATLAS-master\\reference-regions\IPCC-WGI-reference-regions-v4_shapefile\IPCC-WGI-reference-regions-v4.shp', encoding = 'gb18030')
IPCC_shapes = shapefile.Reader('E:\Data_Evaluation\ATLAS-master\\reference-regions\IPCC-WGI-reference-regions-v4_shapefile\IPCC-WGI-reference-regions-v4.shp')
...
for xNew, yNew in border_points:
    listx1.append(xNew)
    listy1.append(yNew)
#xpoint = [8.7175, 127.2506, 140.0269, -96.4701, 121.2653]
#ypoint = [45.0700, 38.2013, 36.0539, 41.1649, 14.1412]
...
boundary = gp.GeoDataFrame.from_file('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\\Example_final_final_final.shp', encoding = 'gb18030')
data_geod = gp.GeoDataFrame.from_file('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\\Example_final_final_final.shp', encoding = 'gb18030')
# plot 1
f = plt.figure(figsize = (12, 7), dpi=100)  # initiate the figure
...
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\ObsIWW.png')
#plt.show()

#plot 2
f2 = plt.figure(figsize = (6, 7), dpi=100)  # initiate the figure
...
h2 = plotmap(boundary, ax1, 'Bias_ctl', cmap_bias1, norm_bias1,legend_kwds={'label': r"mean bias ($km^3$/yr)",'orientation': "horizontal"})
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\CTL_Obs.png')

#plot 3

f2 = plt.figure(figsize = (12, 7), dpi=100)  # initiate the figure
f2.subplots_adjust(hspace=0.1, wspace=0.1, left = 0.05, right = 0.95, top = 0.95, bottom = 0.05)
...
ax4.set_title('IRR_3 - OBS',loc='right',fontsize = 12)
ax4.text(0.01,0.92,'d',color='dimgrey',fontsize=12, transform=ax4.transAxes, weight = 'bold')

h1 = plotmap(boundary, ax1, 'Bias_irr', cmap_bias1, norm_bias1,legend_kwds={'label': r"mean bias ($km^3$/yr)",'orientation': "horizontal"})
h2 = plotmap(boundary, ax2, 'Bias_drai', cmap_bias1, norm_bias1,legend_kwds={'label': r"mean bias ($km^3$/yr)",'orientation': "horizontal"})
h3 = plotmap(boundary, ax3, 'Bias_pool', cmap_bias1, norm_bias1,legend_kwds={'label': r"mean bias ($km^3$/yr)",'orientation': "horizontal"})
h4 = plotmap(boundary, ax4, 'Bias_satu', cmap_bias1, norm_bias1,legend_kwds={'label': r"mean bias ($km^3$/yr)",'orientation': "horizontal"})
plt.savefig('E:\Data_Evaluation\Data_for_evaluation\Irrigation_amount\Fig\\IRR_CTL.png')
#plt.show()
...
ax1 = subplotdefi(2,2,1,'',14)
ax1.set_title('MAE(IRR_0) - MAE(CTL)',loc='right',fontsize = 12)
ax1.text(0.01,0.92,'a',color='dimgrey',fontsize=12, transform=ax1.transAxes, weight = 'bold')
#ax2 = subplotdefi(2,2,2,'Mean absolute error (MAE) change (IRR_drai - CTL)',14)
ax2 = subplotdefi(2,2,2,'',14)
ax2.set_title('MAE(IRR_1) - MAE(CTL)',loc='right',fontsize = 12)
ax2.text(0.01,0.92,'b',color='dimgrey',fontsize=12, transform=ax2.transAxes, weight = 'bold')
#ax3 = subplotdefi(2,2,3,'Mean absolute error (MAE) change (IRR_pool - CTL)',14)
ax3 = subplotdefi(2,2,3,'',14)
ax3.set_title('MAE(IRR_2) - MAE(CTL)',loc='right',fontsize = 12)
ax3.text(0.01,0.92,'c',color='dimgrey',fontsize=12, transform=ax3.transAxes, weight = 'bold')
#ax4 = subplotdefi(2,2,4,'Mean absolute error (MAE) change (IRR_satu - CTL)',14)
ax4 = subplotdefi(2,2,4,'',14)
ax4.set_title('MAE(IRR_3) - MAE(CTL)',loc='right',fontsize = 12)
ax4.text(0.01,0.92,'d',color='dimgrey',fontsize=12, transform=ax4.transAxes, weight = 'bold')
